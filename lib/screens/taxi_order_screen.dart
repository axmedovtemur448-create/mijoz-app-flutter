...
class _TaxiOrderScreenState extends State<TaxiOrderScreen> {
  ...
  // TEZ ishlashi uchun: avval telefon xotirasidagi so'nggi ma'lum joylashuvni
  // darhol ko'rsatamiz, keyin fonda aniqrog'ini yangilaymiz.
  Future<void> _detectCurrentLocation() async {
    try {
      ...
      // 1) Tezkor: oxirgi ma'lum joylashuv (deyarli bir zumda)
      ...
      // 2) Aniq: haqiqiy joriy joylashuv (bir necha soniya)
      final pos = await Geolocator.getCurrentPosition(
        locationSettings: const LocationSettings(accuracy: LocationAccuracy.medium),
      ).timeout(const Duration(seconds: 6), onTimeout: () => lastKnown ?? (throw Exception("timeout")));
      ...
      if (_fromLatLng != null) {
        final address = await _reverseGeocode(_fromLatLng!);
  ...
  Future<String> _reverseGeocode(LatLng point) async {
    try {
      ...
      final res = await http.get(uri, headers: {"User-Agent": "mijoz_app_flutter"}).timeout(const Duration(seconds: 5));
