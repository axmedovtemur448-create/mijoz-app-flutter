...
class _TaxiOrderScreenState extends State<TaxiOrderScreen> {
  ...
  // Serverdan kelgan haqiqiy tariflar (bo'lmasa - standart ro'yxat ishlatiladi)
  List<Map<String, dynamic>> _tariffs = [
    {"id": "start", "name": "Start", "base_price": 8000, "price_per_km": 1500},
    {"id": "komfort", "name": "Komfort", "base_price": 10000, "price_per_km": 2200},
    {"id": "biznes", "name": "Biznes", "base_price": 15000, "price_per_km": 3500},
  ];
  ...
  Future<void> _loadTariffs() async {
    final real = await ApiService.fetchTariffs();
    if (real.isNotEmpty && mounted) {
      setState(() => _tariffs = real);
    }
  }

  num _priceFor(Map<String, dynamic> t) {
    final base = (t["base_price"] ?? 0) as num;
    final perKm = (t["price_per_km"] ?? 0) as num;
    final dist = _distanceKm > 0 ? _distanceKm : 5; // manzil hali tanlanmagan bo'lsa, taxminiy
    return (base + perKm * dist).round();
  }
  ...
  Future<void> _updateRoute() async {
    ...
    final result = await fetchRoute(_fromLatLng!, _toLatLng!);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _callDriver() async {
    ...
    final tariff = _tariffs[_selectedVehicle];
    final price = _priceFor(tariff);

    final order = await ApiService.createOrder(
      pickupLat: from.latitude,
      pickupLng: from.longitude,
      pickupAddr: _fromAddress,
      destLat: to.latitude,
      destLng: to.longitude,
      destAddr: _toLatLng == null ? "Belgilanmagan" : _toAddress,
      tariff: (tariff["id"] ?? "start").toString(),
      price: price,
    ...
    if (order == null || order["id"] == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Serverga ulanib bo'lmadi. Qaytadan urinib ko'ring.")),
      );
      return;
    }
    ...
    Navigator.of(context).push(MaterialPageRoute(
      builder: (_) => SearchingDriverScreen(
        ...
        tariffName: (tariff["name"] ?? "Start").toString(),
        price: "$price so'm",
      ),
    ));
  }
            ...
            child: FlutterMap(
              ...
              children: [
                    ...
                    if (_fromLatLng != null)
                      Marker(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xFF3B9DE8).withOpacity(0.25)),
                          child: Container(
                            width: 18,
                            height: 18,
                ...
                if (_routePoints.length > 1)
                  PolylineLayer(
                    polylines: [
                      Polyline(points: _routePoints, color: const Color(0xFFFFC800), strokeWidth: 4),
                    ],
                  ),
              ],
            ),
                  ...
                  children: [
                      ...
                      children: List.generate(_tariffs.length, (i) {
                        ...
                        return Expanded(
                          child: GestureDetector(
                            ...
                            child: Container(
                              margin: EdgeInsets.only(right: i != _tariffs.length - 1 ? 8 : 0),
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Text("${t["name"]}", style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                                  Text("~${_priceFor(t)} so'm", style: const TextStyle(color: Colors.white54, fontSize: 11)),
                                ],
                              ),
                            ),
                          ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: _creatingOrder ? null : _callDriver,
                        ...
                        child: _creatingOrder
                            ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF1B1B1B)))
                            : const Text("Taksi", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ),
                    ),
                  ],
                ),
